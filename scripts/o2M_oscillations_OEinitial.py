...
# eccentricity = 0.00 for the cleanest CW/OE comparison; swap to 4.03e-3 later
base_kepler = dict(
...
# ── Deputy initialization: δa=0, δe=A/a (in-plane bounded ellipse) ───────────
deputy_kepler = build_deputy_from_oe_offsets(
    base_kepler,
    da = 0.0,
    de = A / base_kepler["semi_major_axis"],
)
...
# OE_osc[name] must be shape (6, N)
OE_osc = {}
for name in sat_names:
    r_arr, v_arr = rv[name]
    oes = np.array([rv_to_kepler(r_arr[k], v_arr[k]) for k in range(N)])  # (N, 6)
    OE_osc[name] = oes.T  # (6, N)

# Quick δa drift diagnostic over simulation
...
# 1. LVLH R/S/W components vs time
...
# 3. Osculating orbital elements — the 'a' panel is the key δa=0 check
